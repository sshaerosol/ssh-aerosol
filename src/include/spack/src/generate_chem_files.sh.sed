...
# 1: h2o
# 2: user-defined
if [ $is_combining == 1 ]; then
    scheme=h2o
elif [ $is_combining == 2 ]; then
    scheme=user
fi


$script_dir/combine_files.py $SPECIES $REACT $scheme
if [[ $? = 1 ]]; then
    exit 1
fi
...
output_file="LU_decompose.f90 LU_solve.f90 species-list.dat dratedc.f90 fexchem.f90 fexloss.f90 fexprod.f90 jacdchemdc.f90 kinetic.f90 rates.f90 dimensions.f90 non_zero.dat"
