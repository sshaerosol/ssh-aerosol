...
namespace AtmoData
{
  ...
  */
  template <class T>
  CountryEmission<T>::CountryEmission(T emission, int country):
  ...
  */
  ...
  void ReadEmep(Date date, const vector<string>& Sp_emis_names,
		...
		Data<list<CountryEmission<real> >, 4, real>& Emis_land,
		Data<list<CountryEmission<real> >, 4, real>& Emis_water)
  {
	...
	while (has_element(EmepEmisStream))
	  {
		...
		if ((Ncountry >= 30 && Ncountry <= 35) || Ncountry == 70)
		  Emis_water(l, j-1, i-1, s).
		    push_back(CountryEmission<real>(quantity *
							monthly * daily /
							365, Ncountry));
  ...
  //! Reads EDGAR reformatted files and applies monthly and daily factors.
  /*!
    First reads EDGAR standard files located in the input directory (format
    <species>.dat). In these files, the EDGAR emissions (Tons) correspond to a
    given year, and are provided for world countries and for each sectors.
    Then applies monthly and daily coefficients in order to provide emissions
    for a given day.
    \param date date.
    \param Sp_emis_names inventory emission species names.
    \param input_directory input directory.
  ...
  */
  template <class real>
  void ReadEDGAR(Date date, const vector<string>& Sp_emis_names,
		 string input_directory, string input_file,
		 const Data<real, 3>& MonthlyFactors,
		 const Data<real, 3>& DailyFactors,
		 const real& deposition_factor_nh3,
		 Data<list<CountryEmission<real> >, 4, real>& Emis_edgar)
  {
    int Ncountry_max = MonthlyFactors.GetLength(0);
    int Ncountry, i, j, s, id;
    string country, line, sector, species;
    vector<string> v;
    real quantity, monthly, daily;

    int Nsp_emis = Emis_edgar[0].GetLength();
    int Ny_edgar = Emis_edgar[2].GetLength();
    int Nx_edgar = Emis_edgar[3].GetLength();
  
    RegularGrid<real> GridSectors(Emis_edgar[1]);
    ...
    for(int l = 0; l < Nsp_emis; l++)
      {
	string emis_file_edgar = input_directory + Sp_emis_names[l] + ".dat";
	ExtStream EdgarEmisStream(emis_file_edgar);
	if (!EdgarEmisStream.is_open())
	  throw string("File ") + emis_file_edgar + " doesn't exist.";

	while (has_element(EdgarEmisStream))
	  {
	    EdgarEmisStream.GetLine(line);
	    v = split(line, ";");

	    species = v[3];
	    int name_lenght = species.size();
		...
		n++;
		if (n >= int(CountryCode.size()))
		  throw string("Error in ReadEdgar: country code \"") + country
		    + string("\" not found in \"") + input_file + "\".";
	      }
	    Ncountry = CountryNumber[n];
	...
	if (EdgarEmisStream.bad())
	  throw string("EDGAR emission file \"") + emis_file_edgar
	    + "\" is badly formatted.";
      }
  }
	...
	while ( has_element(EmepEmisStream) )
	  {
	    ...
	    if ( Ncountry >= 30 && Ncountry <= 35 )
	      for (int s = 0; s < Nsectors; s++)
		Emis_water(l, j-1, i-1, s).push_back(CountryEmission<real>(Emis_emep(s)
	    ...
	    // In land.
	      ...
	      for (int s = 0; s < Nsectors; s++)
		Emis_land(l, j-1, i-1, s).push_back(CountryEmission<real>(Emis_emep(s)
									      * MonthlyFactors(Ncountry, s, date.GetMonth()-1)
									      * DailyFactors(Ncountry, s, day) / 365.,
									      Ncountry));
	  }
  ...
  */
  ...
  void EmepToLatLon(const Data<int, 2, real>& LUC,
		    ...
		    Data<list<CountryEmission<real> >, 4, real>& Emis_land,
		    Data<list<CountryEmission<real> >, 4, real>& Emis_water,
		    Data<list<CountryEmission<real> >, 4, real>& Emis_out)
  {
    int l, i, j, s;
    typename list<CountryEmission<real> >::const_iterator iter;
    typename list<CountryEmission<real> >::iterator iter_out;
		  ...
		  for (l = 0; l < Nsp_emis; l++)
			   ...
			   iter != Emis_water(l, j_emep, i_emep, s).end(); ++iter)
			{
			  ...
			  if (iter_out != Emis_out(l, s, j_polair, i_polair).end())
			    ...
			    Emis_out(l, s, j_polair, i_polair).push_back(CountryEmission<real>(iter->emission_ * Ratio, iter->country_));
			}
		}
	      else
		{
  ...
  /*!
    Computes emissions on a latitude-longitude grid (like Polair3D grid) on
    the basis of a coarser latitude-longitude grid.
    \param Emis_in emissions (Tons).
    \param Emis_out (output) output emissions (Tons . m^{-2}) for a given day.
  */
  template <class real>
  void LatLonToLatLon2(Data<list<CountryEmission<real> >, 4, real>& Emis_in,
		       Data<list<CountryEmission<real> >, 4, real>& Emis_out)
  {
    int l, i, j, s;
    typename list<CountryEmission<real> >::const_iterator iter;
    ...
    int Nsp_emis = Emis_in[0].GetLength();
    int Nsectors = Emis_in[1].GetLength();
    int Ny_in = Emis_in[2].GetLength();
    real y_min_in = Emis_in[2](0);
    real Delta_y_in = (Emis_in[2](Ny_in-1) - y_min_in) / real(Ny_in-1);

    int Nx_in = Emis_in[3].GetLength();
    real x_min_in = Emis_in[3](0);
    real Delta_x_in = (Emis_in[3](Nx_in-1) - x_min_in) / real(Nx_in-1);

    int Ny_out = Emis_out[2].GetLength();
    real y_min_out = Emis_out[2](0);
    real Delta_y_out = (Emis_out[2](Ny_out-1) - y_min_out) / real(Ny_out-1);

    int Nx_out = Emis_out[3].GetLength();
    real x_min_out = Emis_out[3](0);
    real Delta_x_out = (Emis_out[3](Nx_out-1) - x_min_out) / real(Nx_out-1);

    real Delta2_x_in = Delta_x_in / 2.0;
    real Delta2_y_in = Delta_y_in / 2.0;
    real Delta2_x_out = Delta_x_out / 2.0;
    real Delta2_y_out = Delta_y_out / 2.0;
  
    real left_border = x_min_out - Delta2_x_out;
    real right_border = left_border + Nx_out * Delta_x_out;
    real bottom_border = y_min_out - Delta2_y_out;
    real top_border = bottom_border + Ny_out * Delta_y_out;

    // Browsing of the simulation cells.
    for (j = 0; j < Ny_out; j++)
      for (i = 0; i < Nx_out; i++)
	{
	  //XXX
	  cout << j << " " << i << endl;
	  //XXX
	  // Lat/Lon coordinates of the center of the simulation cell.
	  real lon_out = (x_min_out + i * Delta_x_out);
	  real lat_out = (y_min_out + j * Delta_y_out);

	  // Index of the emission cell at the bottom left of the
	  // center of the simulation cell.
	  int i_bl = int ((lon_out - x_min_in) / Delta_x_in);
	  int j_bl = int ((lat_out - y_min_in) / Delta_y_in);
	  //XXX
	  cout << j_bl << " " << i_bl << endl;
	  cout << "i_bl" << " " << lon_out << " " << x_min_in << endl;
	  //XXX
	  
	  // Lat/Lon coordinates of the center of the bottom left
	  // emission cell.
	  real lon_bl = (x_min_in + i_bl * Delta_x_in);
	  real lat_bl = (y_min_in + j_bl * Delta_y_in);

	  real ratio_right =
	    (lon_out + Delta2_x_out - lon_bl - Delta2_x_in) / Delta_x_out;
	  real ratio_left =
	    (lon_bl + Delta2_x_in - lon_out + Delta2_x_out) / Delta_x_out;
	  real ratio_top =
	    (lat_out + Delta2_y_out - lat_bl - Delta2_y_in) / Delta_y_out;
	  real ratio_bottom =
	    (lat_bl + Delta2_y_in - lat_out + Delta2_y_out) / Delta_y_out;
	  
	  // Normalisation of the ratios.
	  // 1 - The simulation cell is splitted over the emission cells
	  // if the ratios are positive. A negative ratio implies the
	  // simulation cell does not intersect the corresponding
	  // emission cells.
	  Cut(ratio_right);
	  Cut(ratio_left);
	  Cut(ratio_top);
	  ...
	  // 2 - A ratio over one implies the simulation cell is fully
	  // included in the corresponding emission cells.
	  ratio_right = min(ratio_right, 1.);
	  ratio_left = min(ratio_left, 1.);
	  ratio_top = min(ratio_top, 1.);
	  ratio_bottom = min(ratio_bottom, 1.);

	  for (l = 0; l < Nsp_emis; l++)
	    for (s = 0; s < Nsectors; s++)
	      {
		for (iter = Emis_in(l, s, j_bl, i_bl).begin();
		     iter != Emis_in(l, s, j_bl, i_bl).end(); ++iter)
		  {
		    iter_out = Emis_out(l, s, j, i).begin();
		    // Browse countries already present in the
		    // list Emis_out(l, s, j_bl, i_bl).
		    while (iter_out != Emis_out(l, s, j, i).end()
			   && iter_out->country_ != iter->country_)
		      ++iter_out;
		    // If the current country is met, add to
		    // the previous value.
		    if (iter_out != Emis_out(l, s, j, i).end())
		      iter_out->emission_ +=
			iter->emission_ * ratio_left * ratio_bottom;
		    // Else add an element to the list.
		    else
		      Emis_out(l, s, j, i).push_back
			(CountryEmission<real>
			 (iter->emission_ * ratio_left
			  * ratio_bottom, iter->country_));
		  }
		for (iter = Emis_in(l, s, j_bl, i_bl+1).begin();
		     iter != Emis_in(l, s, j_bl, i_bl+1).end(); ++iter)
		  {
		    iter_out = Emis_out(l, s, j, i).begin();
		    // Browse countries already present in the
		    // list Emis_out(l, s, j_bl, i_bl).
		    ...
		    if (iter_out != Emis_out(l, s, j, i).end())
		      iter_out->emission_ +=
			iter->emission_ * ratio_right * ratio_bottom;
		    // Else add an element to the list.
		    else
		      Emis_out(l, s, j, i).push_back
			(CountryEmission<real>
			 (iter->emission_ * ratio_right
			  * ratio_bottom, iter->country_));
		  }
		for (iter = Emis_in(l, s, j_bl+1, i_bl).begin();
		     iter != Emis_in(l, s, j_bl+1, i_bl).end(); ++iter)
		  {
		    iter_out = Emis_out(l, s, j, i).begin();
		    // Browse countries already present in the
		    // list Emis_out(l, s, j_bl, i_bl).
		    ...
		    if (iter_out != Emis_out(l, s, j, i).end())
		      iter_out->emission_ +=
			iter->emission_ * ratio_left * ratio_top;
		    // Else add an element to the list.
		    else
		      Emis_out(l, s, j, i).push_back
			(CountryEmission<real>
			 (iter->emission_ * ratio_left
			  * ratio_top, iter->country_));
		  }
		for (iter = Emis_in(l, s, j_bl+1, i_bl+1).begin();
		     iter != Emis_in(l, s, j_bl+1, i_bl+1).end(); ++iter)
		  {
		    iter_out = Emis_out(l, s, j, i).begin();
		    // Browse countries already present in the
		    // list Emis_out(l, s, j_bl, i_bl).
		    ...
		    if (iter_out != Emis_out(l, s, j, i).end())
		      iter_out->emission_ +=
			iter->emission_ * ratio_right * ratio_top;
		    // Else add an element to the list.
		    else
		      Emis_out(l, s, j, i).push_back
			(CountryEmission<real>
    ...
    const real ratio_pi = pi / 180.;
    const real surface_factor = ratio_pi * ratio_pi
      * Earth_radius * Earth_radius * Delta_x_out * Delta_y_out;
    real surface;
      ...
      for (j = 0; j < Ny_out; j++)
	{
	  surface = surface_factor
	    * cos((y_min_out + j * Delta_y_out) * ratio_pi);
	  for (l = 0; l < Nsp_emis; l++)
	    for (s = 0 ; s < Nsectors; s++)
	      for (iter_out = Emis_out(l, s, j, i).begin();
		   iter_out != Emis_out(l, s, j, i).end(); ++iter_out)
