      ...
      for (i=0;i<nmols;i++)
        {
          theta(i)/=sumtheta;
          phi(i)/=sumphi; 
          //cout << phi(i)/Xmol(i) << " " << Z/2*Qparam(i) << " " << theta(i)/phi(i) << " " << Lparam(i) << " " << phi(i)/Xmol(i) << " " << Lmean << endl;
          //cout << log(phi(i)/Xmol(i))+Z/2*Qparam(i)*log(theta(i)/phi(i))+Lparam(i)-phi(i)/Xmol(i)*Lmean << endl;
