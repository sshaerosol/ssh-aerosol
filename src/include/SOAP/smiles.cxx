...
void get_smiles(model_config &config, vector<species>& surrogate)
{
  ...
  for (i=0;i<n;i++)
	...
	Array<int, 2> carbon_cycle,carbon_ether,carbon_ketone;	
	int igr,igr2;
	...
	while (igr<ngr)
	  {
	    ...
	    if (surrogate[i].smile.substr(igr,2)=="C1")
	      {
	...
	while (surrogate[i].smile!="")
	  {	  
	    ...
	    else if (surrogate[i].smile.substr(0,3)=="OOC")	      
	      {
		...
		if (is_double==1 and nc_double_save>=0)
		...
		else if (will_be_double==1)
		  {
		    ...
		    if (nC1==2 and nC2==1)
		      ...
		      {
			cout << "error double bounds type" << endl;
			break;
		      }
		    will_be_double=0;
		    nc_double_save=-1;
