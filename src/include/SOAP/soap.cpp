...
extern "C" void soap_main_(double& LWC, double* RH, double* Temperature, 
                           ...
                           double* liquid_Nsize, int* nbin,int* isoapdyn, int* soap_inorg,
                           char* species_name, int* name_len, double* molecular_weight_aer,
                           double* accomodation_coefficient, double *diffusion_coef, int* nlayer,
  ...
  return soap_main(LWC, *RH, *Temperature, 
                   ...
                   *ns_aer, *ns_aer_layers, *neq, q, qaero, qgas,
                   ...
                   liquid_Nsize, *nbin, *isoapdyn, *soap_inorg,
               ...
               int activity_model, double epser_soap)
{
  ...
  parameters(config, surrogate, species_list_aer, molecular_weight_aer,
             accomodation_coefficient, diffusion_coef);
      ...
      for (i = 0; i < n; ++i)
	{
	  ...
	  if (iq != -1)
	      ...
	      if (surrogate[i].is_organic)
		{
	      ...
	      else if (surrogate[i].is_inorganic_precursor) //inorganic gas concentrations
		{
      ...
      if (config.compute_inorganic)
	{
      ...
      for (i = 0; i < n; ++i)
	      ...
	      else if (surrogate[i].is_organic==false and i!=config.iHSO4m)
		{
                  ...
                  for (b = 0; b < config.nbins; ++b)
		    {
		      surrogate[i].Aaq_bins_init(b) = q[iq_aero + b]; //*surrogate[config.iNH3].MM/surrogate[config.iNH4p].MM;
		      if (surrogate[i].name=="SO4")
		  ...
		  //cout << "in: " << surrogate[i].name << " " << surrogate[i].Aaq_bins_init << endl;
		}
      ...
      for (i = 0; i < n; ++i)
	      ...
	      else if (i!=config.iHSO4m) 
		{
		  ...
		  if (i==config.iSO4mm)
		    for (b = 0; b < config.nbins; ++b)
		      {
			q[iq_aero + b]*=surrogate[config.iH2SO4].MM/surrogate[config.iSO4mm].MM;
			q[iq_aero + b]+=surrogate[config.iHSO4m].Aaq_bins(b)/surrogate[config.iHSO4m].MM*surrogate[config.iH2SO4].MM;
		      }
		  else if (i==config.iNH4p)
		    for (b = 0; b < config.nbins; ++b)
		      q[iq_aero + b]*=surrogate[config.iNH3].MM/surrogate[i].MM;
		  else if (i==config.iNO3m)
		    for (b = 0; b < config.nbins; ++b)
		      q[iq_aero + b]*=surrogate[config.iHNO3].MM/surrogate[i].MM;
		  else if (i==config.iClm)
		    for (b = 0; b < config.nbins; ++b)
		      q[iq_aero + b]*=surrogate[config.iHCl].MM/surrogate[i].MM;
		      
		  //cout << "out: " << surrogate[i].name << " " << q[iq_aero] << endl;
		}
            }
        }
