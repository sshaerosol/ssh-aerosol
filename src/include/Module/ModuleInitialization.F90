...
!    parameter (n_reaction = 197, n_photolysis = 24)
    ...
    integer :: tag_thrm!method for wet diameter computation (0 h2o 1 isorropia)
...
!    DOUBLE PRECISION, dimension(:,:), allocatable :: DLconc_aer
    ...
    character (len=10), dimension(:), allocatable :: emis_species_name
    character (len=100) :: output_directory
    integer :: output_type, ngas_emis, naero_emis
    ...
    namelist /init_data/ init_bin_number
    namelist /aerosol_size/ N_sizebin
 ...
 contains
   ...
   subroutine read_configuration(configuration_file)
     ...
     read(11, nml = aerosol_size)
     write(*,*) "N_sizebin", N_sizebin
     allocate(init_bin_number(N_sizebin))

     read(11, nml = init_data)
     write(*,*) "init_bin_number:", init_bin_number
     ...
     ! Count the number of emitted gas-phase species.
     count = 0
     ierr = 0
     open(unit=12, file = emisgas_file, status = "old")
     do while(ierr .eq. 0)
        read(12, *, iostat=ierr) sname, temp
        if (ierr == 0) count = count + 1
     end do
     close(12)
     ngas_emis = count
     write(*,*) "Number of emitted gas-phase species:", ngas_emis
     allocate(gas_emis(ngas_emis))
     allocate(emis_species_name(ngas_emis))

     ! Read the gas-phase emissions.
     open(unit=12, file = emisgas_file, status = "old")
     do s = 1, ngas_emis
        read(12, *) emis_species_name(s), gas_emis(s)
     end do
     close(12)
     ...
     !read initial information of each species, one species one line
     do s= 1, N_species
        if(Tag_init.eq.0) then
           read(11,*) List_species(s),ic_name, Index_groups(s),init_mass(s)
        else
           read(11,*) List_species(s),ic_name, Index_groups(s),&
                (init_bin_mass(k,s),k=1,N_sizebin)
           do k=1,N_sizebin
     ...
     open(unit=12, file = emisaero_file, status = "old")
     do s=1,min(N_species,2)
        read(12,*) (init_bin_emission(k,s),k=1,N_sizebin)
     enddo
     close(12)
...
!      ncycle_aer = 1
    ...
    if (tag_thrm .eq. 1) then ! Isorropia
