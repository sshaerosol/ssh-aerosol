	...
	for y in range(10):
		...
		for theta in range(9):
			...
			for day in range(Ndays):
                                output.write("Day = " + str(NDAY[day]) + '\n')
                                d = 1
#				Mean = J[:,y,theta,day].mean()
				Mean = J[0,y,theta,day].mean() # at the ground
				PI     = 3.141592653589793
      				PI180  = PI/180.0

      				YGRDJ=MLAT[y]*PI180 # conversion of latitude to radian
#                                SDA = 23.45*sin((0.9863*NDAY[day]-81)*PI180)
                                SDA_accurate = asin(sin(23.45*PI180)*sin((0.9863*NDAY[day]-81)*PI180)) # Sun declination angle in radian
...
#plot(SZA_temp, J_temp, "k-o", label=str(d))
mean_SZA_temp_2D = []
mean_J_temp_2D = []
for theta in range(9):
