...
SSH-aerosol is written in the programming language FORTRAN and C++. It can run on PC or a cluster with both the gfortran and the GNU gcc compiler and under a Linux system. Before the compilation, make sure the construction tool: SCONS has already been installed.
If not you can obtain it through the instruction of the site: \\
...
\begin{itemize}
	...
	\item Subfolder {\it{TM}}: this subfolder contains files that record the time variation of total mass concentration ($\mu g/m^3$) and totoal aerosol mass concentration of all size sections ($\mu$g~m$^{-3}$). The files is named after the aerosol species name, (followed by the name of its gas phase percursor for recording total mass) and followed by '\_TM'.\\ 
...
\begin{figure}[H]
        \begin{center}
                \includegraphics[angle=0,width=0.95\textwidth]{fig/array.png}
        \end{center}
...
The variable {\textit{DTAEROMIN}} specfies the The minimum time step (in seconds) that can be used in the solver. 
...
illustate this effect, the condensation of hydrophobic organic surrogates of different volatilities is studied for different
...
\caption{Viscosity test case: time evolution of organic concentrations (SOAlP
  ...
  several organic-phase diffusivities: 10$^{-18}$ (dark blue), 10$^{-19}$ (light
