...
SSH-aerosol is written in the programming language FORTRAN and C++. It can run on PC or a cluster with the GNU compilers (gfortran, gcc and g++) under a Linux system. Intel compiler should also work. If not, please report to  \url{ssh-aerosol-help@liste.enpc.fr}.\\

The following external libraries are required:
\begin{itemize}
\item C++ library Blitz++ (\url{https://github.com/blitzpp/blitz}). See section~\ref{sec:blitz} for the instruction.
...
\url{http://www.unidata.ucar.edu/downloads/netcdf/index.jsp}. See section~\ref{sec:netcdf} for the instruction.
\item SWIG (\url{http://www.swig.org/index.php}). See section~{sec:swig} for the instruction.
\item Header file for Python (Python.h). It is included in \code{libpython3-dev}. When you install \code{python3-dev}, \code{libpython3-dev} is installed too.
\end{itemize}
The random access memory (RAM) requirement for a 0D simulation is very small. 


All installations are done on your local directory \code{YourLocal}, for example,  \code{~/usr/local}.
\begin{verbatim}
export YourLocal=~/usr/local
\end{verbatim}

\subsection{Install Blitz++}\label{sec:blitz}

If you install Blitz++ using a package manager like apt or yum, you need to install \code{libblitz0-dev} and \code{libblitz0v5}.

If you can not install using a package manager, you can install from source files. To do that, \code{cmake} is required to build Blitz++ from source files.

If \code{cmake} is not installed, this following guide provides instruction on how to install.
You can skip if \code{cmake} is already installed.
\begin{verbatim}
...
- to build \code{Blitz++} from the source
\begin{verbatim}
...
And edit \code{CMakeCache.txt} to indicate where blitz is installed. 
Please indicate the full path to YourLocal
...
Please set the following paths to the installed \code{Blitz++} in your environment before compiling.
\begin{verbatim}
...
\subsection{Install netCDF}\label{sec:netcdf}

You have to install \code{netCDF-C} and then \code{netCDF-fortran}.

\subsubsection{\code{netCDF-C}}

If you install \code{netCDF-C} using a package manager like apt or yum, you need to install \code{libnetcdf-dev}.
...
\subsubsection{\code{netCDF-fortran}}

If you install \code{netCDF-fortran} using a package manager like apt or yum, you need to install \code{libnetcdff-dev}.
...
./configure CPPFLAGS="-I$HOME/usr/local/include" LDFLAGS="-L$HOME/usr/local/lib" --prefix=$HOME/usr/local
make
make install
\end{verbatim}


\subsection{\code{SWIG}}\label{sec:swig}

If you install \code{SWIG} using a package manager like apt or yum, you need to install \code{swig}.

If you can not install using a package manager, you can install from source files.
...
% Any version of \code{Lapack/Blas} should be installed.
% If Lapack/Blas is installed by the package manager, \code{PATH\_TO\_LAPACK} should be \code{/usr/lib}


\subsection{Compilation with scons}\label{sec:scons}
...
After all the required software and library are ready, you may need to modify \code{compile} to indicate the right path.
...
Finally, the compilation can be done by typing a simple command: \code{compile}.
\begin{verbatim}
./compile
\end{verbatim}
...
concentrations. An example is provided in the file \textit{test\_upsample.py}.

The following scheme is employed : We assume the mass concentration M and the number concentration N of section, ranging from diameters $d_{min}$ to $d_{max}$, are related such that $M = \frac{\pi}{6} \rho \bar{d}^3 N$ where $\rho$ is the density of the aerosols in that section and $\bar{d}=\sqrt{d_{min}d_{max}}$ is the geometric mean diameter of the section boundaries.

Provided that $\{d_i\}$ is a partition of the interval $[d_{min},d_{max}]$, such that $d_{i-1}<d_i$ for all $i = 1, ..., n$ and $d_0=d_{min}$, $d_n=d_{max}$, we can derive a mass and number concentrations that satisfy the condition $M_i = \frac{\pi}{6} \rho \bar{d_i}^3 N_i$, where $\bar{d_i}=\sqrt{d_{i-1}d_{i}}$ is the geometric mean diameter of the $i$-th section, in a way that conserves both total mass and number.

By setting

\[
M_i =  M \dfrac{d_i^{3/2} - d_{i-1}^{3/2}}{d_n^{3/2} - d_0^{3/2}}
\]

\noindent one can easily check that $\sum_{i=1}^n M_i = M$ and $\sum_{i=1}^n N_i = N$.
...
\begin{equation}
k_{oligo}=k^{max}_{oligo} a_{monomer} %^{m_{oligo}-1}
\label{oligo_kin}
\end{equation}
