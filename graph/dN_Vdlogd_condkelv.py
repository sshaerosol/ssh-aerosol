...
################### input parameters #################################"
pcase1= '../results/condkelv/ref-dyn/'
pcase2 = '../results/condkelv/nokelv-dyn/'
pcase3 = '../results/condkelv/ref-eq/'
pcase4 = '../results/condkelv/nokelv-eq/'
sizebin_ssh = 50
...
for i in cases :
        for j in range(sizebin_ssh) :
                ...
                with open (pcase+'/aero/PSOAmP_' + str(j+1) + '.txt') as finit :
                        values = finit.read().splitlines()
                mass_init_sml[cases.index(i)][j] = float(values[0])
                mass_out_sml[cases.index(i)][j] = float(values[-1])
                with open (pcase+'/aero/PSOAlP_' + str(j+1) + '.txt') as finit :
                        values = finit.read().splitlines()
                mass_init_sml[cases.index(i)][j] += float(values[0])
                mass_out_sml[cases.index(i)][j] += float(values[-1])
